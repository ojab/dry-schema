...
RSpec.describe Dry::Schema, "callbacks" do
  ...
  context "when schema with callbacks is reused" do
    ...
    let(:nested_schema) do
      Dry::Schema.define do
        ...
        before(:key_coercer) do |result|
          Marshal.load(result.output)
        end
      end
    end

    it 'correctly handles reused schemas' do
      nested_schema_input = Marshal.dump(name: "ojab")

      expect(first_schema.(a: nested_schema_input).to_h).to eql(a: {name: "ojab"})
      expect(second_schema.(b: {foo: nested_schema_input}).to_h).to eql(b: {c: {name: "ojab"}})
    end
  end
  ...
  context 'when input is not a Hash' do
      ...
      Dry::Schema.define do
        required(:email).schema do
          required(:address).schema do
            ...
            before(:key_coercer) { |result| Marshal.load(result.output) }
          end
    ...
    specify do
      input = Marshal.dump(email: Marshal.dump(address: Marshal.dump(local_part: "ojab")))
